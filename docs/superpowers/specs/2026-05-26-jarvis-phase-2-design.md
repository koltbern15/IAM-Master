...
1. **JARVIS-grade visual identity** — a cinematic, tactical-holograph aesthetic replacing the restrained dark-slate shell built in Plan 1 (Foundation).
...
| `<ModuleConstellation>` | The home page centerpiece. Composes 12 `<ModuleNode>` (small circular nav buttons) in a clockwise ring around a `<RadialRing>` core. Cursor parallax on nodes. |
...
- Centerpiece: `<ModuleConstellation>` — compact `<RadialRing>` showing total mastery % at center, 12 `<ModuleNode>`s arrayed in a clockwise ring at fixed 30° increments starting from top. Phase 1 nodes cyan, Phase 2 amber, Phase 3 dim. Cursor parallax: nodes shift up to 4px based on mouse position. Click → `/modules/[id]`. Animated rotating dashed rings behind nodes (10s and 14s opposite rotations).
...
- All 5 animated flow diagrams from §9
...
- 3D / WebGL diagrams — already cut from Phase 1, stays cut
...
| Bundle budget | `next build` output | Framer Motion + Howler + `cmdk` + Fuse.js combined ≤ 200 KB gzipped. Warn at 180 KB. |
...
3. `/` renders the Module Constellation with all 12 nodes positioned correctly, status strip live, ticker scrolling, mastery core animated.
...
8. All 5 animated flow diagrams play with cyan-palette token animation along paths.
...
13. Bundle size under 200 KB gzipped for the new interaction dependencies.
